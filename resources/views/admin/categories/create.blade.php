...
@section('content')
    <section class="rounded-3xl border border-blue-600/30 bg-white/5 p-8 shadow-2xl shadow-blue-900/10">
        <div class="mb-8">
            <p class="text-xs font-black uppercase tracking-[0.3em] text-blue-400">Nueva categoría</p>
            <h1 class="mt-3 text-4xl font-black uppercase tracking-tight text-white">Crear Categoría</h1>
        </div>
            ...
            <div class="rounded-3xl border border-blue-600/30 bg-black/70 p-6 shadow-lg shadow-blue-900/10">
                <div class="space-y-4">
                    <label class="block">
                        ...
                        <input name="name" value="{{ old('name') }}" class="mt-2 w-full rounded-3xl border border-blue-600/30 bg-black/80 px-4 py-3 text-white focus:border-blue-400 focus:outline-none @error('name') border-red-500 @enderror" required>
                        @error('name')
                    ...
                    <label class="block">
                        ...
                        <input name="icon" value="{{ old('icon', '📁') }}" class="mt-2 w-full rounded-3xl border border-blue-600/30 bg-black/80 px-4 py-3 text-white focus:border-blue-400 focus:outline-none" maxlength="10">
                        <p class="mt-1 text-xs text-gray-500">Ej: 🍔 🍕 🥗 🍟 🥤</p>
                    </label>

                    <label class="flex items-center gap-3">
                        <input type="checkbox" name="is_active" value="1" @checked(old('is_active', true)) class="h-4 w-4 rounded border-blue-600/30 bg-black/80 text-blue-600">
